...
class OracleProcessor:
    ...
    def __determine_price_for_kalman(self, past_prices: list, raw_price: float) -> tuple[bool, float]:
        ...
        window_for_stats = past_prices[-(self.window_size - 1):] if len(
            past_prices) >= (self.window_size - 1) else past_prices
    ...
    def initialize_or_load_kalman(self, item: Item, price_for_kalman: float) -> float:
        if item.kalman_state_x and item.kalman_state_p:
            try:
                self.kalman_filter.set_state_from_json(
                    item.kalman_state_x, item.kalman_state_p)
                previous_kalman_estimation = self.kalman_filter._kf.x[0, 0]
            except Exception as e:
                logging.error(
                    f"Failed to load Kalman state for item {item.id} from JSON: {e}.")
    ...
    async def get_kalman_prediction(self, item_id: int, steps: int = 5) -> tuple[float, float] | None:
        ...
        if not item or not item.kalman_state_x or not item.kalman_state_p:
            logging.warning(
                f"Could not get Kalman state for prediction for item {item_id}. State x: {item.kalman_state_x}, State P: {item.kalman_state_p}")
            return None
        ...
        try:
            ...
            temp_kalman_for_prediction.set_state_from_json(
                item.kalman_state_x, item.kalman_state_p)
            predicted_price, predicted_trend = temp_kalman_for_prediction.predict_state(
                steps=steps)
            return predicted_price, predicted_trend
        except Exception as e:
            logging.error(
                f"Failed to predict Kalman state for item {item_id}: {e}")
            return None
    ...
    async def process_notifications(self, item: Item):
        ...
        signal_percent = (
            (predicted_price - item.current_price) / item.current_price) * 100
    ...
    async def update_item_price(self, item_id: int, raw_price: float, volume: int):
        ...
        if not item:
            logging.warning(
                f"Item with ID {item_id} not found. Skipping price update.")
            return
        ...
        is_outlier, price_for_kalman = self.__determine_price_for_kalman(
            past_prices, raw_price)
        if is_outlier and len(history_entries) >= (self.outlier_persistence_threshold - 1):
            past_outliers = [
                h.is_outlier for h in history_entries[:self.outlier_persistence_threshold - 1]]
            if all(past_outliers):
                is_outlier = False
                price_for_kalman = raw_price

        previous_kalman_estimation = self.initialize_or_load_kalman(
