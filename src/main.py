...
async def process_and_update_prices(session: AsyncSession, processor: OracleProcessor, fetched_data: list):
    for data in fetched_data:
        ...
        stmt = select(Item).where(Item.market_hash_name ==
                                  market_hash_name_from_steam)
        result = await session.execute(stmt)
        item = result.scalar_one_or_none()

        if not item:
            logging.warning(
                f"Item with market_hash_name '{market_hash_name_from_steam}' not found in DB for update. Skipping.")
            continue
        ...
        try:
            logging.info(
                f" Updating price for Item ID {item_id} (Price: {raw_price}, Volume: {volume})")
            await processor.update_item_price(item_id, raw_price, volume)

            updated_item = await session.get(Item, item_id)
            if updated_item:
                logging.info(
                    f"Item {item_id} (DB Updated): Raw Price={updated_item.current_price:.2f}, Oracle Price={updated_item.oracle_price:.2f}, Trend={updated_item.trend:.4f}")
                prediction_steps = 5
                ...
                if predicted_data:
                    ...
                    logging.warning(
                        f"Failed to get prediction for item {item_id}.")
            else:
                logging.warning(
                    f"Could not retrieve updated item {item_id} after processing (item might have been deleted).")

        except Exception as e:
            logging.error(
                f"Error processing item ID {item_id} (Name: {item_name_from_steam}, Price: {raw_price}): {e}", exc_info=True)
...
async def run_parser_loop(session: AsyncSession):
    ...
    while True:
        try:
            ...
            logging.info(
                f"Fetching data for market_hash_names: {market_items}")
            ...
            if fetched_data:
                async with AsyncSessionLocal() as write_session:
                    processor = OracleProcessor(
                        sql_session=write_session, bot=bot)
                    await process_and_update_prices(write_session, processor, fetched_data)
            else:
                logging.info(
                    "No valid data fetched from Steam for any item. Waiting...")
    ...
    async with AsyncSessionLocal() as parser_read_session:
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"Error waiting for parser task to finish: {e}", exc_info=True)
